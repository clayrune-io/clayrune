# CLAUDE.md — archived long-form sections

**Archived 2026-08-06.** `CLAUDE.md` is auto-loaded into EVERY
session for this project, so its size is a per-session context tax on every
agent, forever. It had reached 34 KB / ~8,800 tokens — 1.5x the 24 KB budget we
deliberately enforce on `MEMORY.md` (`index_byte_budget`), with no budget of its
own.

Roughly half of it was **narrative about work that has since shipped**: build
orders, phase plans, and committee status for the Skills-Curation / learning
system, all of which now live in their own specs under `docs/`. That material is
history, not instruction — it belongs where it can be read on purpose, not
prepended to every task.

**Nothing here was deleted, only moved.** This file is the verbatim state of
`CLAUDE.md` immediately before the trim. The binding rules, live gotchas and
load-bearing invariants were kept in `CLAUDE.md` itself, unchanged.

One section was **retired rather than archived for reuse**: the `memsearch`
instructions. That plugin was verified non-functional and retired 2026-05-18,
but `CLAUDE.md` went on telling every agent in every session to use it. The
2026-08-06 night review flagged the contradiction explicitly. Do not reinstate.
