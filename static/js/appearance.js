...
// ── Built-in backgrounds (gallery) ──────────────────────────────────────
// Shipped under static/backgrounds/, generated by tools/generate-backgrounds.py
// (keep ids in sync with the files it writes). Selecting one stores its URL in
// mc_bg_image — same pipeline as a custom image, minus the data-URL re-encode,
// so dim + crop/framing work unchanged and localStorage stays tiny.
const BUILTIN_BGS = [
  { id: 'doodle-dark',   label: 'Doodles · Dark',   w: 2560, h: 1440 },
  { id: 'doodle-aurora', label: 'Doodles · Aurora', w: 2560, h: 1440 },
  { id: 'doodle-warm',   label: 'Doodles · Warm',   w: 2560, h: 1440 },
];

function _builtinBgUrl(id, thumb) {
  return '/static/backgrounds/' + id + (thumb ? '-thumb' : '') + '.webp';
}

function _bgIsBuiltin() {
  return (localStorage.getItem('mc_bg_image') || '').startsWith('/static/backgrounds/');
}

function setBuiltinBg(id) {
  const bg = BUILTIN_BGS.find(b => b.id === id);
  if (!bg) return;
  localStorage.setItem('mc_bg_image', _builtinBgUrl(bg.id));
  localStorage.setItem('mc_bg_imgw', String(bg.w));
  localStorage.setItem('mc_bg_imgh', String(bg.h));
  bgZoom = 100; bgPosX = 50; bgPosY = 50;
  localStorage.setItem('mc_bg_zoom', '100');
  localStorage.setItem('mc_bg_posx', '50');
  localStorage.setItem('mc_bg_posy', '50');
...
window.BUILTIN_BGS = BUILTIN_BGS;
window._builtinBgUrl = _builtinBgUrl;
window._bgIsBuiltin = _bgIsBuiltin;
window.setBuiltinBg = setBuiltinBg;
