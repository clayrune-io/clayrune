...
| POST | `/api/project/<project_id>/agent/<session_id>/job` | MC-958 follow-up (b49cf71f): run `{"command":...,"cwd":optional,"timeout_minutes":optional,"shell":optional}` as a real OS subprocess Clayrune owns, for engines with no background-job facility of their own (Codex/Gemini/Qwen — one process per turn, exits at turn end). `shell` is `"bash"` (default, every OS) \| `"sh"` \| `"powershell"` \| `"cmd"`, argv-invoked — on Windows, `"bash"`/`"sh"` resolve to Git Bash (rejecting the WSL launcher at `System32\bash.exe`) and fall back to powershell if none is found; unknown values 400. Returns `{job_id, pid, shell, shell_note}`. On exit, delivers a follow-up turn into the SAME session (exit code, duration, output tail, log path, resolved shell) via the delegation-delivery outbox/inbox — works even if the session's own process has already exited. 404 if the session doesn't exist or belongs to another project; 400 for incognito sessions. `timeout_minutes` defaults to and is capped by `background_wait_max_minutes` (120). |
...
### Delegation delivery (feature-branch implementation)

These endpoints require the durable delivery service initialized at startup;
they return 503 when unavailable. They do not certify task-result acceptance.
...
| GET | `/api/project/<project_id>/agent/delegation/status-list?limit=50&offset=0` | Read-only project-scoped recovery metadata plus `usage` advisory: aggregate UTF-8 `payload_bytes`/`row_count` and per-table `completion_sources`/`outbox`/`inbox` counts; `usage.status=unknown` on read failure. Never includes task/result payloads, and warning never gates delivery. |
...
inbox entry means the continuation was handed off, not that it succeeded.
